...
function populateSidebar(items) {
  ...
  let sidebarOuterContainer = document.querySelector(
  ...
  let sidebarItemsContainer = sidebarOuterContainer.querySelector(
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.addEventListener("click", (event) => {
    ...
    if (
      ...
      !optionsIcon.contains(event.target)
    ) {
    ...
    globalPapersData.forEach((paper) => {
      ...
      const titleClass =
        paper.figures && paper.figures.length > 0
          ? "paper-title-with-figures"
          : "paper-title-no-figures";
      const abstractClass =
        paper.figures && paper.figures.length > 0
          ? "paper-abstract-with-figures"
          : "paper-abstract-no-figures";
  ...
  function applyCurrentFilterAndPopulate() {
    ...
    let papersToShow =
      currentFilter.type === "all"
        ? globalPapersData
        : globalPapersData.filter(
            (paper) => paper[currentFilter.type] === currentFilter.value,
          );
  ...
  function populateViewBasedOnCurrentFilter() {
    const isTableVisible = !document
      .querySelector(".table-container")
      .classList.contains("hidden");
    const isCardsVisible = !document
  ...
  function filterPapersBasedOnCurrentFilter() {
    if (currentFilter.type === "all") {
      ...
      return globalPapersData.filter(
        (paper) => paper[currentFilter.type] === currentFilter.value,
      );
    }
  }
  ...
  function populateDetailsPanel(paper) {
    ...
    if (paper) {
      ...
      function populateDetailsPanel(paper) {
        ...
        const dropdownCitationMenu = document.getElementById(
          "dropdown-citation-menu",
        );
      ...
      paper.figures.forEach((fig, index) => {
        ...
        const figureNameThumb = fig
          .substring(fig.lastIndexOf("/") + 1, fig.lastIndexOf("."))
          .replace(/_/g, " ")
          .replace(/fig/gi, "fig ");
        ...
        thumb.addEventListener("click", () => {
          ...
          const subfigurePart = fig.match(/([a-zA-Z]+)\./)
            ? fig.match(/([a-zA-Z]+)\./)[1]
            : "";
          const fullFigureLabel = fig
            .substring(fig.lastIndexOf("/") + 1, fig.lastIndexOf("."))
            .replace(/_/g, " ")
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  $(document).ready(function () {
    ...
    $(document).on("contextmenu", ".paper-card", function (event) {
      ...
      console.log("Custom context menu shown for ID:", paperId);
      console.log(
        "Custom context menu shown for ID:",
        $("#customContextMenu").data("targetId"),
      );
    });
    ...
    $("#addStar").on("click", function () {
      ...
      if (paper) {
        ...
        console.log(
          `Starred status toggled for paper with ID: ${targetId}. Starred: ${paper.starred}`,
        );
      }
    ...
    $("#addBookmark").on("click", function () {
      ...
      if (paper) {
        ...
        console.log(
          `Bookmarked status toggled for paper with ID: ${targetId}. Bookmarked: ${paper.bookmarked}`,
        );
      }
    ...
    $("#addArchive").on("click", function () {
      ...
      if (paper) {
        ...
        console.log(
          `Archived status toggled for paper with ID: ${targetId}. Archived: ${paper.archived}`,
        );
      }
    ...
    $("#addThesis").on("click", function () {
      ...
      if (paper) {
        ...
        console.log(
          `Thesis status toggled for paper with ID: ${targetId}. Thesis: ${paper.thesis}`,
        );
      }
...
function getRecentlyViewedPapers(globalPapersData) {
  ...
  console.log(
    "Filtered recently viewed papers:",
    globalPapersData.filter((paper) => recentlyViewedIds.includes(paper.id)),
  );
  return globalPapersData.filter((paper) =>
    recentlyViewedIds.includes(paper.id.toString()),
  );
}
